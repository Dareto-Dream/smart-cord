...
/// <summary>
/// "Now playing" — Spectralis's OBS overlay when it's running (track, progress, a
/// mini spectrum from its visualizer levels, today's scrobble tally), else whatever
/// Windows' own media session broker reports (any app with System Media Transport
/// Controls — Spotify desktop, a browser tab, VLC). The generic source has no
/// spectrum/scrobble data, so those bits are hidden rather than shown as zeroes.
/// </summary>
public sealed class NowPlayingScreen : IPixooScreen
{
    ...
    public string Label => "Now playing";
    ...
    private void RenderPlaying(PixooCanvas c, PixooContext ctx, Sources.NowPlayingSample n, Color accent)
    {
        ...
        ScreenChrome.Footer(c, ctx.Now, n.Source == "Spectralis" ? $"{n.ScrobblesToday} today" : null);
    }
