...
public sealed class PixooService : IDisposable
{
    ...
    private IReadOnlyList<IPixooScreen> _allScreens;
    ...
    public string CustomScreensDirectory => CustomScreenLoader.Directory;
    ...
    private static IReadOnlyList<IPixooScreen> BuiltInScreens() =>
        [new StatusScreen(), new NowPlayingScreen(), new ComputeScreen(), new GpuScreen(), new SystemScreen()];

    /// <summary>Re-scans <see cref="CustomScreenLoader.Directory"/> — pick up new/edited/removed
    /// custom screens without restarting the app.</summary>
    public void ReloadCustomScreens()
    {
        _allScreens = BuiltInScreens().Concat(CustomScreenLoader.LoadAll(_logger)).ToList();
        Changed?.Invoke();
    }
    ...
    /// <summary>
    /// While true, the rotation is suspended and the panel stays blank (streaming /
    /// do-not-disturb mode) instead of resuming on the next tick the way a one-shot
    /// <see cref="BlankAsync"/> does.
    /// </summary>
    public void SetForceBlank(bool blank)
    {
        _forceBlank = blank;
        _ = RenderAndPushAsync(force: true);
    }
    ...
    private PixooContext BuildContext() => new(
        ...
        _gpu.Latest, _system.Latest, ResolveNowPlaying(), _gpu.Compute,
        DateTime.Now);

    /// <summary>Spectralis's richer feed (spectrum, scrobbles) wins when it's running;
    /// otherwise fall back to whatever Windows' media session broker reports.</summary>
    private Sources.NowPlayingSample? ResolveNowPlaying() =>
        _spectralis.Latest ?? FromMediaSession(_controller.NowPlaying);

    private static Sources.NowPlayingSample? FromMediaSession(Signals.MediaSnapshot? m) =>
        m is null || !m.HasTrack
            ? null
            : new Sources.NowPlayingSample(
                Title: m.Title, Artist: m.Artist, Album: m.Album,
                PositionSeconds: m.PositionSeconds, DurationSeconds: m.DurationSeconds,
                IsPlaying: m.IsPlaying, Levels: [], Accent: "#1DB954",
                ScrobblesToday: 0, ScrobblesTotal: 0, MinutesToday: 0,
                At: m.CapturedAtUtc, Source: "MediaSession");
    ...
    private async Task RenderAndPushAsync(bool force)
    {
        ...
        try
        {
            ...
            _lastPushUtc = DateTimeOffset.UtcNow;
            LastResult = _forceBlank ? $"blanked (streaming) {DateTime.Now:HH:mm:ss}" : $"{screenId} · pushed {DateTime.Now:HH:mm:ss}";
        }
